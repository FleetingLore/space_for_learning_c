...
const double const_4 = 2.5; // 小时变化系数
const int const_5 = 24; // 用水时间
const int const_6 = 8; // 每户设置的卫生器具当量数
const double const_7 = 2.5; // K_h
...
f_3_input_float() {
    double output_float;
    int _1 = scanf("%lf", &output_float); // [stdio.h]
    return output_float;
}
...
f_4_input_int() {
    int output_int;
    int _1 = scanf("%d", &output_int); // [stdio.h]
    return output_int;
}
...
test_1() {
    ...
    double target_u_0 = f_1_(q_0, total_persons, k_h, total_n_g, t);
...
main() {
    ...
    // 人数和用水时间可以由用户自己输入
    printf("请输入人数 m: "); // [stdio.h]
    int m = f_4_input_int();
    
    printf("请输入用水时间 T: "); // [stdio.h]
    double t = f_3_input_float();
    ...
    double target_u_0 = f_1_(q_0, m, k_h, total_n_g, t);
    f_2_display(target_u_0);
